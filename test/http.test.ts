...
test("rejects an unsupported protocol with BundesratNetworkError", async () => {
...
test("timeoutMs bounds the whole response, not just idle gaps", async () => {
  ...
  await withServer(
    (_req, res) => {
      ...
      const finish = setTimeout(() => res.end("</r>"), 2000);
      res.on("close", () => {
        clearInterval(drip);
        clearTimeout(finish);
      });
    ...
    async (baseUrl) => {
      ...
      await assert.rejects(
        ...
        (err) => err instanceof BundesratNetworkError && /timed out after 300ms/.test(err.message),
      );
